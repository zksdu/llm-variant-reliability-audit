...
## 十、最终全量结果（6 模型 × 5000 变异，2026-08-06 完成）

### 实验规模
- 30,000 次 LLM 调用：deepseek-v4-pro / deepseek-chat / deepseek-coder / Kimi-K2.6 / MiMo V2.5 Pro / Qwen3.7-max × 5000 变异
- 4 家厂商（DeepSeek / 月之暗面 / 小米 / 阿里），时间盲法测试集 5000（LastEvaluated ≥ 2026-01）
- 完成度 99.92%（Qwen 4 条网关永久失败缺失，0.08%）

### 总表（全对全 = VUS 计错；表态 = 非 VUS 输出）

| 模型 | 全对全 | 表态准确率 | 表态数 | 金A严(100) |
|---|---|---|---|---|
| Qwen3.7-max | 71.6% | 96.4% | 3715 | 86.0% |
| Kimi-K2.6 | 67.0% | 97.8% | 3422 | 90.0% |
| MiMo V2.5 Pro | 66.1% | 85.2% | 3877 | 92.0% |
| deepseek-v4-pro | 61.8% | 81.2% | 3807 | 93.0% |
| deepseek-chat | 49.4% | 98.6% | 2505 | 69.0% |
| deepseek-coder | 49.2% | 98.7% | 2495 | 68.0% |
| 6 模型共识 | 57.2% | 98.2% | 2647 | 93.5% |

### 四大发现
1. 代际鸿沟：新旗舰 62-72% vs 旧代 49%（+13-22pp）
2. 专家评审档：新旗舰 86-93%（v4-pro 93% 最高）
3. 推理模型敢说但不够稳：v4-pro 81.2%/MiMo 85.2% vs 保守型 ~98.7%
4. 共识反被拖累：6 模型投票 57.2% < Qwen 单模型 71.6%（DeepSeek 3 票保守主导）——模型多样性 > 数量

### 子实验汇总
- ③ ClinGen 专家评审独立集（900 变异）：Kimi 74.7% vs chat/coder ~46%（+29pp）
- ② AF 对照（400 变异）：Benign 敏感度 chat 11%→69%、Kimi 43%→82%；全对全 chat 19%→67%、Kimi 49%→81%——**Benign 弃权主因是信息缺失**
- ④ 冲突子集（44,815 候选）与 MaveDB 功能验证：待全量完成后设计
